...
class roipoly:
    ...
    def __init__(self,func, fig=[], ax=[], roicolor='b'):
        if fig == []:
            fig = plt.gcf()
    ...
    def getVertices(self):
        x = np.round(self.allxpoints,decimals = 0)  
        y = np.round(self.allypoints,decimals = 0)
        vertices = []  
        for i in np.arange(len(x)):
            vertices.append([y[i],x[i]])
        vertices.append([y[0],x[0]])
        return vertices
